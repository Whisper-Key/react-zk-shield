...
const SelectProvider: React.FC<SelectProviderProps> = ({ providerSelectedHandler, network, localAccounts }) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const registry = new WalletProviderRegistry(window, localAccounts, aurolib, network).supportedProviders;
        ...
        <div>
            ...
            <ul>
                {state.providersLoaded && state.providers!.map((provider) => (
                    <li key={provider.name} title={provider.description} data-name={provider.name} style={{ cursor: "pointer"}} onClick={() => connectProvider(provider) }>
